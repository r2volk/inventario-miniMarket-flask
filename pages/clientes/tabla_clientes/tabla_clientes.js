function initTablaClientes() {
  ...
  var paginationEl = document.getElementById("clientes-pagination");
  ...
  document.querySelectorAll(".btn-edit").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      fetch("/api/cliente/" + id)
        ...
        .then(function (c) {
          if (typeof cargarCliente === "function") {
            document.querySelector(".clientes-grid").scrollIntoView({ behavior: "smooth" });
            cargarCliente(c);
          }
        });
    });
  ...
  document.querySelectorAll(".btn-delete").forEach(function (btn) {
    btn.addEventListener("click", async function () {
      ...
      var ok = await confirmar('Eliminar cliente "' + nombre + '"?');
      if (!ok) return;
      fetch("/cliente/" + id, { method: "DELETE", headers: { "X-CSRFToken": document.querySelector('meta[name="csrf-token"]').getAttribute("content") } }).then(function (r) {
        return r.json();
      }).then(function (data) {
        if (data.ok) {
